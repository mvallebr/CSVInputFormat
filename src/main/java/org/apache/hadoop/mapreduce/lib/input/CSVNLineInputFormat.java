...
/**
 * Configurable CSV line reader. Variant of NLineInputReader that reads CSV lines, even if the CSV
 * has multiple lines inside a single column. Also implements the getSplits method so splits are
 * made by lines
 *
 * @author mvallebr
 */
public class CSVNLineInputFormat extends FileInputFormat<LongWritable, List<Text>> {
   ...
   * @param numLinesPerSplit number of lines that should exist on each split
   * @return list of file splits to be processed.
   * @throws IOException
   */
  public static List<FileSplit> getSplitsForFile(
      FileStatus status, Configuration conf, int numLinesPerSplit) throws IOException {
  ...
  public RecordReader<LongWritable, List<Text>> createRecordReader(
    ...
    String quote =
        conf.get(CSVLineRecordReader.FORMAT_DELIMITER, CSVLineRecordReader.DEFAULT_DELIMITER);
    String separator =
        conf.get(CSVLineRecordReader.FORMAT_SEPARATOR, CSVLineRecordReader.DEFAULT_SEPARATOR);
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    ...
    for (FileStatus status : listStatus(job)) {
      List<FileSplit> fileSplits =
          getSplitsForFile(status, job.getConfiguration(), numLinesPerSplit);
